...
def register_output_callback(kernel, agent, function, user_data=None):
    agent.RegisterForRunEvent(sml.smlEVENT_AFTER_OUTPUT_PHASE, function, 
                              user_data)

def register_output_change_callback(kernel, agent, function, user_data=None):
    kernel.RegisterForUpdateEvent(sml.smlEVENT_AFTER_ALL_GENERATED_OUTPUT, 
                                  function, user_data)
...
def print_report_row(mid, user_data, agent, *args):
    ...
    if condition(param_map, domain, agent):
        ...
        pairs.extend("{}={}".format(*reporter(param_map, domain, agent)) 
...
def branch_name(param_map, domain, agent):
    full_path = "{}/SoarSuite/Core".format(env["HOME"])
    result = re.sub(".* ", "", check_output(("ls", "-l", full_path))).strip()
    return ("branch", result)

def avg_decision_time(param_map, domain, agent):
    result = re.sub(".*\((.*) msec/decision.*", r"\1", 
                    agent.ExecuteCommandLine("stats"), flags=re.DOTALL)
    return ("avg_dc_msec", result)

def max_decision_time(param_map, domain, agent):
    result = re.sub(".*  Time \(sec\) *([0-9.]*).*", r"\1", 
                    agent.ExecuteCommandLine("stats -M"), flags=re.DOTALL)
